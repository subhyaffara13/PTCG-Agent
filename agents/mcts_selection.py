"""
agents/mcts_selection.py

Tree-policy helpers for MCTS: UCB calculation and child selection.
"""
...
def calculate_ucb(node: MCTSNode, total_parent_visits: int, c_puct: float) -> float:
...
def select_child(node: MCTSNode, c_puct: float) -> MCTSNode:
    ...
    if node.is_chance_node:
        return _sample_chance_child(node)
    ...
    for child in node.children.values():
        if child.is_pruned:
            continue
        score = calculate_ucb(child, node.visit_count, c_puct)
        if score > best_score:
            best_score = score
            best_child = child
...
def _sample_chance_child(node: MCTSNode) -> MCTSNode:
