...
def has_tag(tags, name: str, bit: int) -> bool:
...
def eval_hand_cards(hand: List[str], registry: Any):
    """Parses hand and computes stats like basic/energy presence, evolution counts."""
    ...
    for cid in hand:
        ...
        if card:
            ...
            if ctype == "Pokemon" or (hasattr(ctype, "name") and ctype.name == "POKEMON"):
                if has_tag(tags, "Basic", 4096): flags["has_basic"] = True
                if has_tag(tags, "Stage 1", 16384): flags["stage1_count"] += 1
                if has_tag(tags, "Stage 2", 32768): flags["stage2_count"] += 1
            elif ctype == "Trainer" or (hasattr(ctype, "name") and ctype.name == "TRAINER"):
                if has_tag(tags, "Supporter", 8192):
                    flags["has_supporter"] = True
                    flags["supporter_count"] += 1
                if has_tag(tags, "search", 1): flags["has_search"] = True
                if has_tag(tags, "draw", 8): flags["has_draw"] = True
                if has_tag(tags, "discard", 128): flags["has_discard"] = True
...
def get_multipliers_and_bonuses(flags, strategy_tips, cfg, supporter_count):
    multiplier = 1.0
...
def eval_bricks_evo(hand: List[str], board: List[str], registry: Any, cfg: dict):
    ...
    for cid in hand:
        ...
        if card and (getattr(card, "card_type", "") == "Pokemon" or getattr(getattr(card, "card_type", None), "name", "") == "POKEMON"):
            tags = getattr(card, "combo_tags", [])
            if has_tag(tags, "Stage 1", 16384) or has_tag(tags, "Stage 2", 32768):
...
def apply_phase_bonuses(phase: str, flags, hand_cards_data, cfg: dict):
    bonus = 0.0
    if phase == 'early':
