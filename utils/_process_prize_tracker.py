...
def _process_prize_tracker(game_state: dict, prize_tracker: Any, packet: Any) -> dict:
    initial_decklist = game_state.get("my_decklist", {})
    if initial_decklist and not getattr(prize_tracker, "initial_decklist", None):
        prize_tracker.record_initial_decklist(initial_decklist)
    hand_ids = game_state.get("my_hand", [])
    if isinstance(hand_ids, list) and hand_ids and getattr(prize_tracker, "initial_decklist", None):
